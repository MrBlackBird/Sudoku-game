...
void read_level(Board &b, std::string level) {
  ...
  while (std::getline(file, line)) {
    ...
    if (std::getline(iss, lev, ',') && std::getline(iss, row_str, ',') &&
        std::getline(iss, nums_str, ',') && std::getline(iss, pos_str, ',')) {
...
int main() {
  ...
  while (game_over == false) {
    ...
    while (true) {
      ...
      // checking input
      if (input_checker(xcord, ycord, num) == true &&
          pos_empty_checker(board, xcord, ycord, num) ==
    ...
    if (check_win(endgame_checker(board), board_full(board)) == true) {
      ...
      time = std::chrono::duration_cast<std::chrono::microseconds>(end - begin)
                 .count();
      game_over_text(turn, time,
                     check_win(endgame_checker(board), board_full(board)));
      game_over = true; // Exit the game loop
    } else if (board_full(board) == true &&
               check_win(endgame_checker(board), board_full(board)) == false) {
      std::chrono::steady_clock::time_point end =
          std::chrono::steady_clock::now();
      time = std::chrono::duration_cast<std::chrono::microseconds>(end - begin)
                 .count();
